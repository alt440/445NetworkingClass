...
class commandInterpreter{
  ...
  */
  ...
  private static final int destinationPort = 8007; //normally 80
  ...
  public static int getHttpMethodIndex(){
  ...
  public static int readInputParameters(String[] args, Request localHttpRequest){
    for(int i=httpMethodIndex+1;i<args.length;i++){
      if(args[i].equals(verboseOption)){
        localHttpRequest.setVerbose();
      } else if(args[i].equals(headerOption)){
        isHeaderOption = true;
      } else if(args[i].equals(inlineDataOption)){
        if(isGetRequest)
        ...
        int returnVal = setAppropriateRequestFlag(args, i, localHttpRequest);
        switch(returnVal){
          case 0: break;
          case -1: return -1;
        }
  ...
  public static int setAppropriateRequestFlag(String[] args, int iteration, Request localHttpRequest){
    if(isHeaderOption){
      localHttpRequest.appendHeaders(args[iteration]);
      isHeaderOption = false;
    } else if(isInlineDataOption && !isD_FOptionUsed){
      String dataSent = formatData(args[iteration]);
      localHttpRequest.setRequestBody(dataSent);
      ...
      localHttpRequest.setDestinationPort(port);
    } else if(isDirOption){
      isDirOption = false;
      localHttpRequest.setDestinationPage(args[iteration]);
    }
